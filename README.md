...
- **Кэширование** — клиентский кэш (localStorage) + проверка commit SHA
...
Для GitHub репозиториев работает двухуровневое кэширование:

1. **Клиентский кэш (localStorage)**
   - Сохраняет результат анализа + commit SHA
   - TTL: 24 часа
   - При повторном анализе сначала проверяет актуальность через `/api/commit-sha`
   - Если commit SHA не изменился — мгновенно возвращает кэш

2. **Серверный кэш (in-memory)**
   - Работает в рамках одного инстанса serverless функции
   - Сбрасывается при деплое

**Как понять что результат из кэша:** в metadata будет `"cached": true` и `analysis_duration_ms: 0`

### GET /api/commit-sha

Получить текущий commit SHA репозитория (для проверки кэша).

```bash
curl "https://git-reps-checker.vercel.app/api/commit-sha?repo_url=https://github.com/user/repo"
```
